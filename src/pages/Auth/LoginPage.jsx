  ...
  return (
    <div className="flex flex-col lg:flex-row bg-stroke-greyBg font-inter p-6 gap-4 items-center justify-center h-screen w-screen">
      ...
      <div className="flex-grow flex flex-col justify-center items-center p-8 bg-white rounded-lg shadow-md relative lg:basis-1/2 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="w-full space-y-4">
          ...
          <div className="flex flex-col items-center space-y-4">
            ...
            <Link to="/forgotpassword" className="text-primaryText text-label-2-medium hover:underline">
              Forgot Password?
            </Link>
          </div>
        </form>
        ...
        <div className="mt-6 text-center">
          ...
          <div className="flex justify-center space-x-6">
            {[{ Icon: FaGoogle, color: 'text-[#DB4437]' }, { Icon: FaApple, color: 'text-[#000000]' }, { Icon: FaFacebookF, color: 'text-[#4267B2]' }].map(({ Icon, color }, idx) => (
              <a key={idx} href="#" className={`${color} hover:opacity-80 transition-opacity duration-300`}>
                <Icon size={24} />
              </a>
            ))}
          </div>
      ...
      <div className="animate-slide-in-right lg:basis-1/2 w-full h-64 lg:h-full rounded-lg overflow-hidden">
        <img src={resort} alt="Relaxing beach resort view" className="h-full w-full object-cover" />
      </div>
    </div>
  );
};
